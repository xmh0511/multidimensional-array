...
{
    Arr(int* pos_ptr = new int(),int* arr_ptr = new int[cal_multi<U,Args...>::value]):Arr<T,Dim,Args...>(pos_ptr,arr_ptr),weight(cal_multi<Args...>::value),_total_size(cal_multi<U,Args...>::value),_pos_ptr(pos_ptr),type_ptr(arr_ptr)
    {
    }
...
struct Arr<T,Dim,U>
{
    Arr(int* pos_ptr = nullptr,int* arr_ptr= nullptr):weight(1),_pos_ptr(pos_ptr),type_ptr(arr_ptr)
    {
        if(_pos_ptr== nullptr){
            single_dim = true;
        }
